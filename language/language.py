...
Parents: {self.parents}\n"""
    ...
    def parse_condition(self, condition: [str]):
        ...
        if condition[1] == "CALLED" or condition[1] == "PID":
            if condition[1] == "CALLED":
                variable = "count"
            else:
                variable = "pid"
        ...
        # Trickiest condition
            ...
            # The precondition is the computation to realise before evaluate the condition
            rust_precondition = ""
            while cursor < len(condition):
                ...
                rust_precondition += f"let arg: usize = ctx.arg({arg_num}).ok_or(0_u32)?;"
                ...
                while separator["type"] != None:
                    ...
                    if separator_type == "->":
                        rust_precondition += "\nlet arg = unsafe { bpf_probe_read_kernel(arg as *const usize).map_err(|_| 1u32)? };"
                ...
                # Othe conditions relative to the arguments remain
                if cursor < len(condition):
                    if condition[cursor] == "AND":
                        rust_condition += " && "
                    elif condition[cursor] == "OR":
